...
resTable = table();
resTable.morphParaNames=parameterNames';
...
writetable(sorted_resTable, './FDR_ephysio_VEN_vs_L56CT_results.xlsx','Sheet','FDR_VENL_vs_VENS','WriteRowNames', true);


% ----------------------------FDR calculation  VENL vs L56CT---------------------------
raw_p = nan(1,nFeature);
for i = 1:nFeature    %get raw p values.
    [raw_p(i)] = ranksum( ephsData_VENL(:,i),ephsData_L56CT(:,i) );
end
%calculate fdr p-values.
[fdr_qvalues] = mafdr(raw_p, 'BHFDR', true);
...
% output the adjusted p-values table to an excel file.
writetable(sorted_resTable, './FDR_ephysio_VEN_vs_L56CT_results.xlsx','Sheet','FDR_VENL_vs_L56CT','WriteRowNames', true);


% ----------------------------FDR calculation  VENS vs L56CT---------------------------
raw_p = nan(1,nFeature);
for i = 1:nFeature    %get raw p values.
    [raw_p(i)] = ranksum( ephsData_VENS(:,i),ephsData_L56CT(:,i) );
end
%calculate fdr p-values.
[fdr_qvalues] = mafdr(raw_p, 'BHFDR', true);
...
% output the adjusted p-values table to an excel file.
writetable(sorted_resTable, './FDR_ephysio_VEN_vs_L56CT_results.xlsx','Sheet','FDR_VENS_vs_L56CT','WriteRowNames', true);