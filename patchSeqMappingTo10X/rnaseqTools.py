...
def map_to_tsne1(referenceCounts, referenceGenes, newCounts, newGenes, clusterIDs,                                 predictedClusterIDs, referenceAtlas, 
                bootstrap = False, knn = 10, nrep = 100, seed = None, batchsize = 100,
                verbose = 1,
                referenceIntronCounts = None, newIntronCounts = None,
                normalizeNew = False, normalizeReference = False,
                newExonLengths = None, newIntronLengths = None):
   
    nPatchCell = newCounts.shape[0]
    pos = np.zeros([nPatchCell,2])
    Corrs = np.zeros([nPatchCell,1])
    cluster_IDs = np.unique(clusterIDs)

    for clusterID in cluster_IDs:
        k=predictedClusterIDs==clusterID
        if np.sum(k)>0:
            patchCounts = newCounts[predictedClusterIDs==clusterID,:]
            refCounts = referenceCounts[clusterIDs==clusterID,:]
            refAtlas = referenceAtlas[clusterIDs==clusterID,:]

            [pos_t,Corr_t]=map_to_tsne(refCounts, referenceGenes, patchCounts, newGenes, refAtlas,
                bootstrap=bootstrap, knn=knn, seed=seed)
            
            pos[predictedClusterIDs==clusterID,:] = pos_t
            print(Corr_t.shape)
            # Corrs[predictedClusterIDs==clusterID] = Corr_t
