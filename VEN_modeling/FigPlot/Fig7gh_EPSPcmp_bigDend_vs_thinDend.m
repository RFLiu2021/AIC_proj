...
fileName ={};
% fileName{end+1} = 'savedData_VENL_synapticInputAtThinSoma(0.5).dat';
% fileName{end+1} = 'savedData_VENL_synapticInputAtThinDend[0](0).dat';
% fileName{end+1} = 'savedData_VENL_synapticInputAtThinDend[0](0.5).dat';
...
% fileName{end+1} = 'savedData_VENL_synapticInputAtBigSoma(0.5).dat';
% fileName{end+1} = 'savedData_VENL_synapticInputAtbigDend[0](0).dat';
% fileName{end+1} = 'savedData_VENL_synapticInputAtbigDend[0](0.5).dat';
...
figure('position',[1000,100,500,1200]);
subplot(3,1,1);
plot(dist, slope(:,1:9),'color',[21,21,211]/255,'LineWidth',2,'marker','o','markerSize',8);hold on;
plot(dist, slope(:,10:end),'color',[255,56,56]/255,'LineWidth',2,'marker','o','markerSize',8);
ax = gca();
ax.XLim = [0,250];
title( 'EPSP slope comparision of BigDend vs thinDend');
ax.XLabel.String = 'Synaptic input distance to Soma (um)';
ax.YLabel.String = 'Rising slope of EPSP';
legend('Thin Dendrite','Thick dendrite');
% paired t-test.
[h,p] = ttest(slope(:,1:9),slope(:,10:end))
% fitting and student's t-test for slopes
stat1 = LinearModel.fit(dist,slope(:,1:9));
p_slope_thin = stat1.Coefficients.pValue('x1');
stat2 = LinearModel.fit(dist,slope(:,10:end));
p_slope_thick = stat2.Coefficients.pValue('x1');

subplot(3,1,2); %figure;
...
subplot(3,1,3); 
plot(dist, delay_peak(:,1:9),'color',[21,21,211]/255,'LineWidth',2,'marker','o','markerSize',8);hold on;
plot(dist, delay_peak(:,10:end),'color',[255,56,56]/255,'LineWidth',2,'marker','o','markerSize',8);
ax = gca();
ax.XLim = [0,250];
ax.YLim = [0,4];
title( 'EPSP peak position comparision of BigDend vs thinDend');
ax.XLabel.String = 'Synaptic input distance to Soma (um)';
ax.YLabel.String = 'EPSP rising time (ms)';
legend('Thin Dendrite','Thick dendrite');
% paired t-test.
[h,p] = ttest(delay_peak(:,1:9),delay_peak(:,10:end))
% fitting and student's t-test for slopes
stat1 = LinearModel.fit(dist,delay_peak(:,1:9));
p_slopeOFrisingT_thin = stat1.Coefficients.pValue('x1');
stat2 = LinearModel.fit(dist,delay_peak(:,10:end));
p_slopeOFrisingT_thick = stat2.Coefficients.pValue('x1');
